...
class Packet:
	def __init__(self, header, stage, latitude, longitude, altitude, imuPacket):
		self.header = header
		self.padding0 = 0
		self.stage = stage
		self.padding1 = 0
...
def pack_data(packet):
	...
	format_string = "BxBxlxlxlxllllll"
	
	# Pack the data using the format string
	packed_data = struct.pack(format_string,
							  packet.header, packet.stage, packet.latitude, packet.longitude, packet.altitude,
							  packet.imuPacket.xGyro, packet.imuPacket.yGyro, packet.imuPacket.zGyro,
							  packet.imuPacket.xAcc, packet.imuPacket.yAcc, packet.imuPacket.zAcc)
...
def prep_next(df,time_step):
	...
	packet_data_class = Packet(0x55, full_data["state"], 0, 0, int(full_data["height"]), imu_data)
